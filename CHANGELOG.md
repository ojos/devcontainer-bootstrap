...
### Summary
- **委譲先エージェント定義を生成物へ配布するようにした**（ojos/ai-packages-dev#259）。`--with-claude` を指定したときだけ `.claude/agents/explorer.md` と `.claude/agents/implementer.md` を置く。雛形は規範パッケージが持ち、本スクリプトは置き先だけを決める（intake 起点スキルと同じ経路）。
- 目的は**モデルとツールを実行環境が読む機構で固定すること**。指示文による呼びかけは迂回できるが、frontmatter は迂回できない。読み取り専用ロールからは編集系ツール（`Edit` / `Write` / `NotebookEdit`）を外す。
- **この版は ai-playbook v0.3.0 以降を要求する**（雛形と、判定の導線を定めた規範の 13 章が必要）。
- 機能追加のみ。既存フラグの挙動と、`--with-claude` 以外の生成物は変わらない。

### Highlights

- **定義だけでは役割は使われない（ojos/ai-packages-dev#259）**: 委譲判定に導線が無ければ、定義があっても選ばれることが構造的に無い。導線は規範側（`shared-ai-rules.md` の「実装委譲パターン」）が持ち、生成物のテストは**規範側に導線が実在すること**まで検査する。
- **`tools` の制約は完全ではない**: 調査に必要なコマンド実行を残す以上、リダイレクトを経由した書き込みは塞げない。機構で保証されるのは編集系ツールの除外までで、そこから先は指示による制約である。この但し書きは各定義に書いてある。
- **既存の生成先は上書きしない**: 衝突ポリシーは既定 `skip`。すでに `.claude/agents/` を持つプロジェクトは影響を受けない。
- **`model` の脱落を検査する**: `model` の記述が落ちた定義は親のモデルを継承し、機構としての意味を失う。落ちたことは実行するまで分からないため、生成物のテストで `inherit` と未指定を落とす。

### 移行

- 既存の生成物は再生成しない限り無影響です。
- 再生成すると、`--with-claude` 指定時に `.claude/agents/` の 2 ファイルが増えます。既存ファイルがあれば `skip` されます。
- **ai-playbook は v0.3.0 以降を指してください。** それより古い版には雛形が無く、生成時に停止します。
